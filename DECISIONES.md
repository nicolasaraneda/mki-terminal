...
## 84. Aplicación del parche de `snapshot.py:140` con su guardia, bump a 5.1.0, y las siete decisiones de la tarde del 8-sep-2026

Contexto: sesión de revisión del 8-sep-2026, después de commitear la corrida 11 (`82f488e`). Las decisiones se tomaron con las tarjetas de `espera_firma.md` y el memo de opciones y consecuencias a la vista. Donde Nicolás no escribió una razón propia, esta acta dice «eligió la opción con la consecuencia declarada en la tarjeta» y no inventa una.

### 84.1 Corte de método en `sesion_objetivo` (§26, acta §82.2 partes a y b)

**Qué se decidió.** `sesion_objetivo` se calcula desde `available_at` (cierre UTC de la sesión del SOX usada) y no desde el reloj de pared del proceso. Diff: `GEMELO/propuestas/parches/snapshot140.diff`, aplicado el 8-sep-2026 y commiteado a las 17:04 hora de Chile, fuera de la ventana de sellado. **Marcador del corte: `plataforma_version >= 5.1.0`.** El primer sello con la semántica nueva es el primero que lleve esa versión; su `timestamp_utc` se lee de `snapshots`, no de esta acta.

**Por qué.** Medido sobre `senales.db` al 2-sep-2026 (n = 295 filas con predicción 4.6.0 en ese momento; la corrida 11 contó 319 al 8-sep, errata registrada en §82.2): 25 filas selladas (4 fechas de emisión: 07-05, 07-29, 08-03, 08-05) apuntan a la sesión siguiente a la que su `available_at` sostiene, porque el sello cruzó la apertura asiática o europea. Las 25 están hoy `verificada` y cuentan en la rama publicada; bajo el ancla correcta las 25 son emisiones tardías por construcción (la sesión correcta había abierto, en 8 casos cerrado, al emitir) y la regla maestra las deja fuera de métricas como `no_verificable_timing`. El defecto seguía activo mientras no se aplicara: cada sello atrasado producía filas nuevas con el mismo error. Ninguna fila nueva desde el 5-ago (los 20 sellos posteriores sellaron entre 22:15 y 23:45 UTC).

**Qué significa la columna a cada lado del corte.** ANTES: «primera sesión del exchange cuya apertura es posterior al reloj de pared al llegar a :140»; coincide con la sesión anticipada sólo cuando el sello no cruza una apertura. DESPUÉS: «primera sesión del exchange cuya apertura es posterior a `available_at`», invariante al atraso del proceso, que es la definición que la regla maestra de `CLAUDE.md` ya daba por cierta. `timestamp_utc` no cambia de significado: sigue siendo el instante real de emisión.

**Cómo se tratan las filas de cada lado.** Ningún análisis mezcla `sesion_objetivo` de antes y de después como la misma medida sin declararlo (misma disciplina que entre `modelo_version`). Las 25 filas viejas quedan selladas tal cual; `available_at` sigue en ellas y permite recomputar la sesión correcta (`backtest/linea_base.sesion_correcta`) cuando un análisis lo necesite. Un cambio de sesión entre dos sellos que rodean el corte por la fórmula nueva no es una anomalía de datos: es este corte operando.

**Qué se descartó y por qué.** Reescribir las 25 filas (prohibido por la Constitución 5.0). Anclar en `timestamp_utc` en vez de `available_at` (reproduce el defecto en el caso del 06-ago, donde el proceso se congeló entre estampar ts y llegar a :140). Corregir el estado de las 25 a `no_verificable_timing` (es reescritura de fila sellada; se declara en el acta, no en el dato).

**Qué queda abierto, actualizado al 8-sep.** Las 15 filas sin pareja ya no están abiertas: se retiraron por el §82.3. La copia de insumos (cola §17 y §16b) sigue abierta. El chequeo del vigía sobre sesiones incoherentes en filas nuevas queda cubierto en parte por el guardia de 84.2; un chequeo específico de `sesion_objetivo` contra `available_at` en filas nuevas sigue propuesto.

**Cómo se revierte.** `git revert` del commit del parche y un segundo corte declarado con la misma forma. Las filas selladas con la semántica nueva no se reescriben tampoco.

### 84.2 El guardia de la rama del `except` (§49, acta §82.2 parte c)

`GEMELO/propuestas/parches/guardia_ancla_temporal.diff` aplicado en el mismo acto que 84.1, como lo exigía el §82.2. Toca `snapshot.py` (aviso en el log cuando `available_at` cae al reloj de pared, por `except` o por `sox_fecha` vacío), `mki_vigia.py` (chequeo `chequear_ancla_temporal`: filas del día con `available_at == timestamp_utc` disparan la alerta) y `senales.py` (el verificador cuenta y declara `sin_calendario`). Forma elegida por la corrida 11 y aceptada: alerta del vigía más log, sin columna nueva, porque la evidencia ya está en la base y una columna es cambio de esquema en filas selladas. La parte (d) del §82.2 quedó hecha en la corrida 11: 0 de 319 filas 4.6.0 pasaron por la rama; el agujero era teórico al aplicarse el guardia. Los 12 tests de `tests/test_parche_snapshot140.py` y `tests/test_parche_guardia_ancla_temporal.py`: 8 pasan sobre el archivo vivo y 4 (los que aplican el diff sobre copias) se saltan solos al detectar el parche aplicado.

### 84.3 `PLATAFORMA_VERSION` 5.0.3 a 5.1.0

Bump en `version.py:40`. Se eligió 5.1.0 y no 5.0.4 porque 84.1 cambia qué significa una columna sellada, no arregla un detalle: cualquier análisis futuro separa filas por este número. Erratas que este bump deja y que se corrigen con nota fechada, no reescribiendo: `CLAUDE.md` línea 7 («currently plataforma 5.0.3») corregida en este mismo commit; `.claude/rules/plataforma.md:26` y `.claude/agents/ingeniero-plataforma.md:52` («5.0.3 quedó congelada al sellar la primera fila») pendientes para la corrida 12; `scripts/ensayo_replica.py:78` lleva `"5.0.3"` en duro y hay que verificar si es un dato de ensayo o una comparación contra la versión viva. Las menciones a «Etapa 5.0.3» (`modo.py`, `mki_backup.py`, `docs/SOMBRA.md`, actas) son nombres de etapa histórica y no cambian.

### 84.4 Siete decisiones para la corrida 12, tomadas con las tarjetas a la vista

Todas se aplican en la corrida 12 (`encargo_corrida_12.md`, sección 2). Se registran acá para que existan en `DECISIONES.md` antes de que la corrida arranque y no sólo en el encargo.

1. **Alcance de la corrida 12: E0 y E1 en papel, sin E2.** La máquina sella su decisión del riel de dinero antes de la apertura con tamaño cero, y en la cuenta de práctica del corredor envía una orden y lee su propia ejecución. Cero dinero real. Se descartó una corrida que opere con dinero porque la regla de aporte exige E0 y E1 con vara pre-registrada antes de E2, y porque nada de lo medido justifica capital (§82.7). Se descartó también sólo cerrar la corrida 11, porque dejaba dos semanas más sin una fila sellada prospectiva del riel, que es lo único que más cómputo no fabrica.
2. **Ruta al corredor: TWS API a través de IB Gateway con la librería oficial `ibapi`.** Dependencia autorizada con versión fijada en `requirements.txt`; el acta de la dependencia la redacta la corrida 12. Se descartó la Client Portal Web API por su sesión que expira, incompatible con una corrida nocturna sin supervisión, y las librerías de terceros por la regla de la casa sobre código sin licencia verificada.
3. **El historial de git no se reescribe.** La atribución de la herramienta se apaga hacia adelante en la configuración local. Razón: la reescritura cambia todos los SHA y convierte cada cita por SHA de las actas y el manual en errata, y el trailer es la señal más débil de las que el repositorio tiene a la vista.
4. **`README.md` pasa a inglés como portada**, generado por la misma plantilla del árbitro; el español se conserva íntegro como `README.es.md`. Ninguna cifra se mueve. Los negativos se publican en inglés con la misma firmeza.
5. **§43: el período de M2 es el horizonte pre-registrado de la vara.** La elección se hizo conociendo qué resultado daba cada alternativa (a 52 semanas ningún juego cruzaba el 25 %; a 156 el juego medio cruza y el juego por defecto no; cifras de la bitácora 11, no citables desde acá). Eso queda declarado y el `estadistico-adversario` dictamina si invalida M2 como vara pre-registrada o sólo obliga a declararlo. Si la vara no tiene horizonte escrito, la decisión no es aplicable todavía y vuelve a la cola con la pregunta exacta.
   > **Nota fechada 9-sep-2026 (corrida 12, bloque 8.1).** La decisión se tomó y la corrida la ejecutó tal como
   > estaba escrita: recomputó M2 sobre la v2 a 52/104/156 semanas con banda entre semillas
   > (`GEMELO/resultados/m2_periodo.md`) y pidió el dictamen. El `estadistico-adversario` la declaró
   > **NO APLICABLE** (`dictamen_12/adversario_43_periodo_m2.md`): el §2 del pre-registro escribe un PISO
   > prospectivo de duración («período mínimo: 52 semanas … hacia adelante»), no un horizonte de acumulación
   > para M2; el «horizonte pre-registrado de la vara» no existe escrito y se dispara la cláusula de escape
   > de este mismo punto. Vuelve a `espera_firma.md` §43 con la pregunta exacta: **¿en qué unidad (el
   > adversario muestra que sólo la tasa anualizada es invariante al período) y contra qué umbral se lee M2,
   > y sobre qué ventana de la cuenta prospectiva?** Además: la elección es inerte para el juego que M2
   > nombra (conservador no cruza en ningún horizonte) y del tipo que el §5 del pre-registro declara
   > ilegítimo (afloja ~3× un criterio de cierre con las tres respuestas a la vista). Enmienda al
   > pre-registro: `preregistro_dinero.md` §8.

6. **§47: piso por posición, acciones enteras.** `SMH`, el benchmark declarado, no cabe con el piso; el riel se compara contra él como línea base sin poder tomar posición, y eso se dice en el README y en la vista. Se descartaron las fraccionarias por su 2 % de ida y vuelta a cualquier tamaño.
7. **E0 cierra con N = 40 filas selladas prospectivas.** Fijado antes de la primera fila; no se mueve. **El monto de E2 no se fija** en esta corrida ni antes de conocer E0 y E1, por la regla de aporte punto 4.

Prerrequisitos de la corrida 12 que siguen siendo actos de Nicolás: cuenta del corredor con usuario de práctica y credenciales en `.env`; IB Gateway instalado y conectable desde WSL; `inventario_abierto_2026-09-07.md` bajado a `GEMELO/resultados/`.

## 85. Corrida 12 (8/9-sep-2026, nocturna): el re-dictamen de la 11, E0 sellado con tamaño cero, el adaptador al corredor contra réplica, la pantalla, y una firma que no se pudo ejecutar

**Contexto.** Encargo en `GEMELO/resultados/encargo_corrida_12.md`; bitácora `bitacora_12.md`; dictámenes en `dictamen_12/`. HEAD al abrir `7f4938a`; el parche del §26 con su guardia estaba aplicado (§84) y el primer sello 5.1.0 existía (2026-09-08 21:15:03 UTC): el bloque 3 no quedó suspendido. Sin credenciales de cuenta de práctica ni gateway del corredor en la máquina (chequeo 0.7, sondeo TCP de lectura): E1 NO EJECUTADO por construcción. Pre-mortem del `director-programa`: 20 instrucciones marcadas, decididas antes del bloque 1 (tabla en la bitácora). Suite al abrir 798 passed / 4 skipped / 1 xfailed.

### 85.1 El re-dictamen: las cifras se sostenían en los artefactos y no en la API

El `estadistico-adversario` juzgó las respuestas congeladas de `/api/rieles`, `/api/dinero/cuenta` y `/api/dinero/universo` (sha256 en `dictamen_12/api_congelada/`). Veredicto: las 18 exigencias del dictamen 11 estaban aplicadas en los artefactos y **ninguna había llegado al serializador de la API**. Cuatro cifras RETIRADAS y en el guardia (`cifras_retiradas.md`, siete patrones): la fricción del juego activo servida como escalar de UNA semilla (12,5) sin intervalo, período ni denominador; la etiqueta «95 %» del IC de σ (cobertura medida 0,850); la cláusula «bajo la regla firmada NO está recomputada» del R2 (falsa desde el 8-sep: +2,6 pp, n=194); y el conteo de huecos del mapa sin presupuesto ni modo. Las 18 exigencias D1–D18 se aplicaron al ejecutable (tabla en la bitácora): la fricción viaja como objeto (mediana de 20 semillas, banda, K, pb, semanas, denominador), la etiqueta del IC se lee del instrumento, el MDE80 se cita con SU σ ancla y nunca en la misma oración que la σ servida (test), `mde80(sigma, T, alpha)` con banda y las dos anualizaciones declaradas, el Wilson iid sobre 480 comparaciones agrupadas RETIRADO (clúster = semilla), cada intervalo con su cobertura medida al lado, el mapa con presupuesto/modo/al_borde/días de censo, `que_lo_mata` compuesto desde el artefacto de coherencia, cobertura del 80 % con Wilson (221/238 → 92,9 % [88,9, 95,5]), etiqueta de régimen (34 snapshots «Alcista · vol alta» y 2 sin régimen sellado), «validado» fuera de generadores y pre-registro, denominadores de la señal larga separados, Wilson del 70,9 % y 56,5 % en coherencia, `BLOQUE_BOOTSTRAP_SEMANAS = 4` fijado por test. **Regla ampliada del bloque de la API (contrato):** el intervalo viaja con su cobertura medida cuando existe, ninguna etiqueta de nivel se escribe a mano, ningún resumen se sirve sin el presupuesto y el modo con que se computó.

**Conteo de intentos (D18).** El adversario no aceptó el 0 del bloque 4 de la corrida 11: la regla de filas `filtrar_sesion_coherente` se evaluó desde la quinta corrida y nunca tuvo fila, y hay precedentes (`COLA`, `DEUDA-2f`). Fila `COHER-12` (2): **352 → 354**; `veredicto_51.N_INTENTOS_PREVIO` 352 → 354 (**358 → 360**). Riel largo 3, sin cambio. El §43 no es intento del DSR (nula conocida); es un grado de libertad de criterio y va a un tercer contador cuya ubicación decide Nicolás.

### 85.2 G3, G8 y el gate en modo diagnóstico

G3: la fuga de 1 día por `precios_ref` se inyecta por un parámetro del riel (`fuga_precios_ref_dias`) y la prueba es de BORDE: el corte se lee de `Libro.decisiones` (primer día en que la fuga cambia una decisión: 2023-09-11, juego agresivo, UMC 2 → 3 acciones) y ahí el gate revienta. Convive con la contraprueba por la señal. G8: `disponibilidad_por_ticker` en el `.meta.json` de todo congelado (primer y último cierre, `available_at_utc` por calendario XNYS vía `exchange_calendars` directo), agregado a los dos existentes con el CSV intacto; deja a la vista que GFS, ARM y SNDK empiezan después de 2018. `verificar_invariancia(..., diagnostico=True)` devuelve el mapa completo de cortes rotos: medido, **5 de 10 cortes por regla ven la fuga de 1 día**. Ninguna cifra publicada se movió.

### 85.3 Los hallazgos del diff, cerrados

Estatus común en `Card` con test que exige la prop en toda tarjeta del riel (y `SIN CIFRAS` explícito para prosa, bloqueante del curador); tragador de `_comisiones_juego_activo` reemplazado por excepciones explícitas con `logging`; el nombre del corredor VUELVE (el curador de la 11 nunca lo exigió; lo escondía un test que ahora prohíbe hablar con un corredor, no nombrar la fuente del arancel); réplicas del bootstrap en UN lugar (`cifras.N_BOOT_DIA = 4000`, el publicado; `bifurcaciones` lo importa; con 10.000 el IC habría sido [−7,2, +26,5]: ninguna cifra se movió); `pd.concat` con `sort=True` explícito en `api/main.py` y `backtest/baselines.py` verificado byte a byte contra una fixture congelada ANTES de editar; erratas de versión con nota fechada (§84.3); `scripts/ensayo_replica.py:78` es dato de ensayo; método de los parches escrito en `docs/manual-agentes.md` con la automatización PROPUESTA (`test_parches_en_worktree.py`).

**Errata fechada 9-sep-2026:** el comentario de `dinero/cuenta_papel.py:8` decía «antes de esta corrida» y dice «antes de la primera corrida»; la corrección es correcta y no tenía acta.

### 85.4 E1: el adaptador contra réplica, y un hallazgo sobre D-C bis

`corredor/ibkr.py` con guardia de papel en código (puerto ∉ {7497, 4002} o cuenta sin prefijo DU → se niega; test con una cuenta que finge ser real), SmartRouting en toda orden, `conciliar()` que declara C2 «NO cumplida» ante una diferencia, y `TransporteReplica` rotulado «escrito desde documentación, NO grabado: NO es evidencia de C1/C2». El transporte real (`ibapi`) queda como `NotImplementedError` a propósito: código no ejecutado no se presenta como cableado. **Hallazgo sobre D-C bis (§84.4.2):** el cliente Python oficial de la TWS API se distribuye desde `interactivebrokers.github.io` (API 10.50, 26-ago-2026) bajo la «TWS API Non-Commercial License» con aceptación previa; el `ibapi` de PyPI es 9.81.1.post1 (dic-2020). Fijarlo en `requirements.txt` instalaría otra cosa: la línea va **comentada** con la versión, e instalarlo es acto de Nicolás (`espera_firma.md` §54). No se revierte la decisión. Los puertos 4001/4002 del Gateway y el prefijo DU salen de un resumen de búsqueda (las páginas del Campus devolvieron 403): declarados NO VERIFICADOS DIRECTAMENTE. Con USD 1.000.000 simulados, E1 validará cableado y no fricción.

### 85.5 La pantalla y la firma §43

Vista `/sellos` y `/api/dinero/sellos`: cada número con estatus, fuente por celda y n; E1 como estado vacío sin datos de ejemplo (test). El `curador-epistemico` RECHAZÓ (4 bloqueantes, 12 correcciones) y todo se aplicó antes del cierre (bitácora, bloque 5). **§47 aplicada** (`regla_aporte` §5-bis: enteras, N = 40, «sin sesión no cuenta», SMH fuera por el piso). **§43 ejecutada como estaba escrita y declarada NO APLICABLE por el adversario** (`dictamen_12/adversario_43_periodo_m2.md`): M2 recomputado sobre la v2 (`m2_periodo.md`) —conservador 3,9 % a 52 semanas, 8,6 % a 104, 12,4 % a 156, no cruza el 25 % en ningún horizonte; medio cruza a 156—, pero el 25 % **no tiene unidad de período** (numerador flujo, denominador fijo en 500 USD) y el «horizonte pre-registrado de la vara» que la firma invoca no existe en el §2 (piso prospectivo de duración). Se disparó la cláusula de escape del §84.4.5 (nota fechada allí); enmienda §8 al pre-registro; la pregunta exacta vuelve a `espera_firma.md` §43. Lectura primaria honesta: la tasa anualizada (3,9 %/año del conservador, estable a los tres horizontes). Cinco de veinte semillas conservadoras se congelan y M2 las puntúa bajo por quiebra operativa. **Un resultado negativo es un resultado.**

### 85.6 Lo que no se hizo

- **Bloque 6 (README en inglés): NO INICIADO.** `README_en_borrador.md` no existe ni en el repo ni en `~`, y no existe ningún generador de README (el árbitro verifica anclas, no genera texto): «generado por la misma plantilla» presuponía dos cosas que la máquina no tiene (pre-mortem 7 y 8). Hacerlo bien exige escribir el generador y extender los guardias a los dos archivos antes de mover un byte; no cabía en la noche sin degradar el resto. D-E sigue firmada y espera la corrida que la ejecute.
- E1 no ejecutado (sin cuenta, sin gateway, NYSE cerrada). La distribución de k bajo la nula para la señal larga (D15) no se computó (`espera_firma.md` §56). El artefacto viejo de `bifurcaciones` no se regeneró.

### 85.7 E0: la primera fila prospectiva del riel de dinero existe

El `auditor-lookahead` dictaminó primero **NO SE SELLA** (cinco fugas demostradas ejecutando código: gate vacuo en el último día, contador dependiente del huso del host, disponibilidad no declarada por ticker, segundo sello silencioso, extensión no versionada; `dictamen_12/auditor_lookahead_sello_dinero.md`), las exigencias E1–E8 se aplicaron (`VERSION_SELLO E0.2`) y re-verificó ejecutando: **«SE PUEDE SELLAR»**. Primer sello a mano el **9-sep-2026 03:38:17 UTC (00:38 Chile)**, fuera de la ventana y antes de la apertura objetivo: 33 filas (una por instrumento operable), `available_at 2026-09-08T20:00Z` por calendario, objetivo 2026-09-09 13:30Z, `pendiente`, insumo fresco y completo, `cuenta_para_N = 1` → **1 de 40**. Decisión del juego por defecto: 0 compras. `senales.db` idéntica antes y después. La fila declara que la señal es la sonda sin información (prueba de maquinaria, no track record) y lleva los sha256 del congelado, de la extensión, de `reglas.json` y del propio sellador.

**Decisiones de diseño de esta corrida, con su razón:** base propia `dinero/sello_dinero.db` y no una tabla en `senales.db` (ningún agente corre DDL sobre la base del titular); el día del sello se deriva del calendario del exchange desde `timestamp_utc` y no del huso del host (dos sellos de la misma información no pueden contar distinto por la hora de pared); un día sin sesión, un sello tardío o un insumo incompleto se sellan igual con su marca y NO cuentan para N (inflar N con filas no verificables cerraría E0 antes de tiempo); la extensión diaria es chica y se respalda en `data/backups/` (el sha sellado tiene que apuntar a algo que exista mañana; los CSV grandes no van al repo cada día); el gate del último día se declara no vigilable en vez de fingirse (un solo corte compara la decisión consigo misma). **Hallazgo del primer sello:** reajuste retroactivo de yfinance en WDC entre el 7 y el 9-sep (dif. rel. 3,2 × 10⁻⁴ sobre 29 sesiones de solape), detectado por E7 y declarado en el meta; no se corrige. Timer del sellador: PROPUESTO en `GEMELO/propuestas/systemd/` (21:00 Chile, Mon..Fri), no instalado (§53).

### 85.8 Errores propios de esta corrida

El sellador E0.1 tenía cinco defectos que el auditor tuvo que demostrar (el gate vacuo era el más grave: la guarda no vigilaba justo donde se la invocaba); la primera versión del test del 2.7 exigía `sort=` en concat que pandas no había marcado; el tragador viejo del test del corredor prohibía nombrar la fuente del arancel; un `Wilson95` viejo quedó en el reporte de la cuenta y rompió la regeneración; el guardia de cifras retiradas no desescapaba `\|` y siete patrones nuevos habrían sido inertes (corregido en `cifras.cifras_retiradas`); el hook de reglas bloqueó dos veces heredocs por su texto y las ediciones se hicieron con la herramienta de escritura; el estatus PROPUESTA en fricción y potencia era el rótulo equivocado (SIMULADO).

### 85.8 bis Normas instaladas de paso (dictamen del `director-programa`), declaradas como tales

1. **`Card` exige `estatus`** (`tests/test_frontend_estatus.py`): alcance real = las vistas listadas en `VISTAS_RIEL`; una vista nueva que no se agregue a la lista no se chequea. Efecto colateral ya cazado: una tarjeta de prosa declara `SIN CIFRAS`, nunca un estatus evidencial inventado.
2. **Censo de palabras sobre el JSON servido** («rentable», «retorno esperado», «ganancia esperada», «confianza», «sostiene casi toda»; E1 sin datos de ejemplo): norma del contrato (`api/CONTRATO.md`), no de la vista.
3. **Un gate de invariancia con un corte vacuo REVIENTA** (`MINIMO_CORTES_GATE = 2` en el sellador; y, generalizado por el director, `cuenta_papel.verificar_invariancia` rechaza cortes en o después del último día de la fuente, con test). Un corte que compara la cuenta consigo misma no vigila nada.
4. **`cifras.cifras_retiradas()` desescapa `\|`**: ensancha el perímetro del guardia para todo `DOCUMENTOS_PUBLICADOS`. Patrones que pasaron de inertes a activos: los siete de esta corrida (los únicos con alternación). Constancia: sobre `DOCUMENTOS_PUBLICADOS` el escaneo da 0 hallazgos; las reintroducciones preexistentes están en `DECISIONES.md`, `espera_firma.md` y `cola_decisiones.md` (fuera del perímetro, en HEAD, declaradas en `ESTADO.md`).
5. **Definición operativa de «sesión que cuenta para N»** (§5-bis de la regla de aporte): definición de agente sobre una cifra firmada; va a confirmación de una línea (`espera_firma.md` §57).
6. **`test_ninguna_funcion_del_riel_manda_ordenes_de_verdad` fue REESCRITO** (de «no nombrar al corredor» a «no hablar con él»), no extendido.

**Inversión de orden contra la prioridad 13 del encargo, declarada:** el bloque 6 (README inglés) iba por delante del 2 y del 8 «si quedaba una hora», y se hicieron el 2 y el 8 completos con el 6 en cero. La razón (las dos premisas del 6 no existían en la máquina) es defendible; el orden se invirtió igual y queda dicho. Y la nota fechada en `bifurcaciones.md` (cinco minutos, nivel 2 de la jerarquía) debió ir antes que un módulo nuevo: se agregó al cierre.
...
## Acta §87, 19-sep-2026 (corrida 13, nocturna). E4-bis cerrada en el sellador de dinero, la sonda del cierre, el README en dos idiomas desde el árbitro, el universo por presupuesto. Ninguna firma

Corrida ejecutada por el orquestador (Fable, esfuerzo alto) desde HEAD `2493bb9`, sábado, arranque 16:24 hora de Chile. Bitácora completa en `GEMELO/resultados/bitacora_13.md`; dictámenes en `GEMELO/resultados/dictamen_13/`. Nada se pushea, nada se firma, nada se opera; el sellador real no se corrió. Todo lo que sigue lleva su etiqueta.

**87.1 (bloque 1, DEMOSTRADO y corregido). La fuga de integridad E4-bis.** `main()` de `dinero/sello_dinero.py` llamaba a `congelar_extension()` antes de `sellar()`, y esa función escribía `dinero/datos/sello/ext_<fecha>.csv` y su meta sin mirar la base. El 10-sep 03:30 UTC el timer disparó sobre el 09-sep ya sellado (sha `126e4f2c…`) con otro insumo (sha `7300787b…`): la base hizo lo correcto (divergencia registrada, cero inserciones) y el archivo sellado quedó pisado nueve días, hasta el `git checkout` de Nicolás del 19-sep. El `auditor-lookahead` escribió el test de reproducción ANTES de la corrección (camino de `main()`, base y carpeta temporales; fallaba sobre el código de la corrida 12) y demostró una segunda fuga que el encargo no traía: con el MISMO insumo también se reescribían los dos archivos (F2, por `mtime`). Dictaminó la opción A: con la fecha ya sellada no se escribe NINGÚN archivo de esa fecha en la carpeta de la evidencia; el insumo divergente no se conserva (temporal fuera de la carpeta, borrado por `main()`) y `divergencias_sello.detalle` declara que su sha apunta a un contenido que ya no existe. Descartada la opción B (archivo `_divergente_`) por abrir una ruta de escritura nueva sobre la carpeta de la evidencia. El orquestador había implementado B antes del dictamen, en el worktree; se descartó (error propio, declarado). Además: `sello_previo()` no filtra por `juego` (R4 del auditor: un cambio de `juego_activo` habría vuelto a escribir una fecha sellada); `--sin-red` sólo levanta `ext_YYYY-MM-DD.csv`; en una divergencia se exporta la tabla y no se respalda nada. Método: worktree temporal desde HEAD con copias de las bases, suite completa ahí (865 passed, 5 skipped, 1 xfailed; el único rojo fue un artefacto gitignorado ausente en el worktree, no el cambio), aplicado al árbol real a las 17:02:11, fuera de la ventana 00:00 a 01:00. `dinero/sello_dinero.db` sha256 `30d4b988…` antes y después; `senales.db` `d476b457…` antes y después. Desde el próximo sello (martes 22-sep 00:30 Chile) las filas llevan `sellador_sha256` `480fbdc9…` (campo declarado E6, no reescritura). Test permanente de integridad en la suite: para cada `fecha_insumo` sellada, el sha256 de su extensión en `dinero/datos/sello/` y en `data/backups/sello_dinero_ext/` es el que la base cita, y el congelado grande es el de `insumo_base_sha256`; salta con razón declarada si no hay base; un rojo significa parar y reportar, no arreglar el archivo. Inferencia, no ejecución: ese chequeo habría fallado entre el 10-sep 00:30 y el 19-sep 16:12.

**87.2 (bloque 2, aplicado). Reintroducciones preexistentes de cifras retiradas.** Etiquetadas con marca fuerte en `cifras.py` (2), `tests/test_bifurcaciones.py` (2, donde el par (72, 56) de la rama derogada es prueba de método y no cifra viva), `espera_firma.md` (3) y `cola_decisiones.md` (6); el escáner da 0 en los cuatro. Las 53 líneas de `DECISIONES.md` son actas históricas y NO se tocan: este archivo no está en `DOCUMENTOS_PUBLICADOS` por diseño. No hizo falta ninguna marca nueva: ninguna norma instalada de paso.

**87.3 (bloque 3, MEDIDO y propuesto). La sonda del cierre.** El 18-sep la única columna vacía a las 23:30 de Nueva York fue TOELY (ADR OTC); en dos descargas por la misma ruta separadas 24 h (extensiones del 16 y del 17-sep) sus cierres del 08 al 15-sep pasaron de estar a estar vacíos (MEDIDO, n = 1 par de descargas, 1 ticker); que el borrado ocurra en Yahoo y no en la ruta de descarga es PROPUESTA sin segunda vía. Con la definición firmada (§86.1) una columna vacía pierde la sesión: 1 pérdida en 9 noches (Wilson 95 % [0,02, 0,44]). Construido: `GEMELO/sonda_cierre.py` (no sella, no toca `dinero/`, no abre bases, test contra la extensión grabada del 18-sep, sin red), `GEMELO/sonda_cierre_resumen.py` (hora mediana y máxima por ticker, hora de todos por noche; probado con tres noches sintéticas) y la unidad PROPUESTA `GEMELO/propuestas/systemd/mki-sonda-cierre.{service,timer}` con `Mon..Fri 17..23:05,35 America/New_York` (desplazada cinco minutos para no coincidir con el sellador de las 23:30: pre-mortem del director) y `Persistent=false`, validada con `systemd-analyze calendar`. NO instalada: instalarla y decidir la hora es de Nicolás (tarjeta §58, tres opciones con consecuencias; recomendación etiquetada: correr la sonda 5 a 10 noches antes de elegir).

**87.4 (bloque 4, aplicado en parte, DECISIÓN PENDIENTE en el resto).** §51, §52, §53 y §57 marcadas firmadas en `espera_firma.md` con referencia a §86 (el curador exigió que las cabeceras dijeran exactamente lo firmado: aplicado). Tarjetas nuevas §58 (hora del sellador), §59 (`visible_en`: la zona ciega Z1 del dictamen 12 no define el campo ni su cálculo; se escribe la pregunta con tres candidatos y no se inventa; no se tocó el sellador una segunda vez la misma noche) y §60 (la opción (a) del hallazgo 2, `.gitignore` para `dinero/datos/sello/`, no consta firmada en ningún documento: **el punto 4.3 NO se ejecutó**). `ESTADO.md` regenerado con el contador leído de la base. **4.5 corrido a las 17:28 y REVERTIDO (hallazgo, no errata).** `python -m GEMELO.bifurcaciones` con las 4.000 réplicas del árbitro movió el ancla como se esperaba, sólo en el último decimal ([−7,2, +26,5] → [−7,2, +26,6], los dos contienen el cero; el acta §78 y la nota fechada del 9-sep ya lo decían), pero la matriz entera cambió por otra causa: el nivel `vivo` del corte se mueve con el reloj y pasó de 251 filas (última señal 27-ago) a 371 (17-sep), y con eso el informe pasa de «0 de 192 celdas con p < 0,05 por clúster» a «20 de 192», y el rango de la ventaja de [−1,1, +15,4] a [−1,1, +20,2] pp (cifras NO DICTAMINADAS por el adversario, intento no contado, fuera del repo; se citan sólo para explicar la reversión). Eso NO es «regenerar con 4.000 réplicas»: es una medición nueva sobre una cadena sellada más larga, que el encargo prohíbe (ninguna afirmación nueva sobre ventaja esta noche) y que exige su propio dictamen del adversario y su lugar en el registro de intentos. El artefacto quedó como estaba en HEAD (10.000 réplicas, con su nota fechada); la corrida de 4.000 sobre la cadena del 17-sep se guardó fuera del repo. Regenerar el artefacto exige primero pinchar el nivel `vivo` a una fecha declarada, o aceptar que el artefacto es una medición nueva: DECISIÓN PENDIENTE de Nicolás (`cola_decisiones.md`).

**87.5 (bloque 5, aplicado). README en dos idiomas, generador primero.** `scripts/generar_readme.py` llena `docs/readme/README.es.tmpl.md` y `README.en.tmpl.md` desde `cifras.sellada()`, `cifras.larga()` y el contador de E0 de `data/backups/sello_dinero.csv`; un marcador sin fuente rompe la generación. La plantilla española se derivó del README.md de esta mañana con 24 sustituciones exactas y reproduce ese archivo byte a byte; `README.es.md` es ese texto más tres cambios declarados: la primera línea de enlace cruzado, «edge» → «advantage» en el TL;DR (curador) y un párrafo nuevo con el negativo de la capturabilidad ya publicado en `estado_epistemico.md` §10 (curador, bloqueante: la página abría un riel de dinero sin su negativo más duro). `README.md` pasa a ser la página en inglés: misma estructura, mismos marcadores más cuatro de E0, sección «Execution rail (paper only)» con E0 en curso, E1 no ejecutado, E2 no iniciado y sin ninguna afirmación positiva; el borrador de Nicolás aportó el tono y una frase de apertura (reescrita por el curador: «Its own errors get caught, dated and published»); sus placeholders sin fuente se descartaron con sus frases. Guardias: `DOCUMENTOS_PUBLICADOS` cubre los dos; `doce_bloques` vigila `README.es.md` y `bloques_readme_en` los nueve en inglés del mismo dict; chequeo 7 de `test_epistemico` sobre los dos; `tests/test_readme.py` (paridad del multiconjunto numérico entre páginas, vocabulario, enlaces, contador). Adversario: cifras IDÉNTICAS inglés ≡ español ≡ árbitro, token a token, incluido el orden. Lo que quedó literal en las plantillas (tablas del WS3/WS4/WS5, holdout, calibración) es deuda declarada del árbitro con procedencia censada en `dictamen_13/adversario_readme.md`, no cifra nueva a mano.

**87.6 (bloque 6, PROPUESTA descriptiva).** `GEMELO/universo_por_presupuesto.{md,json}`, insumo del §52 para la reevaluación del monto de E2 por acta antes de E2: alcance con acciones enteras por presupuesto (7 de 36 a 100 USD, 13 a 250, 29 a 500), membresía en DESDE por techo (18 a 33), semillas conservadoras congeladas antes de 156 semanas (5 de 20 a 500 USD: reproducción determinista de la corrida 12, no verificación) y fricción a 156 semanas condicionada a vivas / congeladas. Rehecho tras cinco exigencias del adversario (`dictamen_13/adversario_universo_presupuesto.md`); no fija ni recomienda ningún monto; la tabla no se lee en columna porque las filas no son pareadas. Registro de intentos del riel largo 3 → 4 (barrido descriptivo, por dictamen).

**87.7 Erratas pendientes de Nicolás, no movidas esta noche (regla: ninguna cifra publicada se mueve sola).** En las dos páginas del README: el N de intentos publica 352/358 y el registro dice 354/360; el «59×» de la ventana larga es 14.618/248 (n de la rama derogada) y con n = 238 es 61,4×; los badges `tests-650` y `plataforma-5.0.3` (hoy 897 tests recolectados y 5.1.0); el n del encabezado de la tabla del holdout (393 / 2.548) no gobierna las celdas E1 (399 / 2.574). `GEMELO/resultados/ventana_larga.md` (líneas 42 y 176) publica vivas dos cifras retiradas porque no está en `DOCUMENTOS_PUBLICADOS`. `data/backups/sello_dinero.csv` va sin commitear: el contador de E0 del README (9 selladas, 7 cuentan, última 2026-09-18) sólo es regenerable desde HEAD cuando se commitee con él.

**87.8 Lo que esta corrida NO hizo.** No corrió el sellador real; no cambió la hora del timer ni §57; no fijó ni movió el monto de E2; no instaló timers ni unidades; no envió órdenes; no afirmó ventaja en ningún idioma; no cableó §46, no firmó `motor_concat.diff`, no tocó §82.7; no escribió el borrador inglés por su cuenta (lo dejó Nicolás); no ejecutó 4.3. Contadores: gap asiático 354, veredicto 5.1 360, sin cambio.

**87.9 Norma instalada de paso, declarada como en §85.8 bis (aceptarla es acto de Nicolás).** El registro de intentos del riel largo pasó de 3 a 4 por un BARRIDO DESCRIPTIVO (9 presupuestos × 20 semillas de la señal sin información), por dictamen del `estadistico-adversario`. El encargo (0.5) y `.claude/rules/backtest.md` definen intento como hipótesis probada o configuración evaluada; una tabla de presupuesto no evalúa ninguna. La dirección es conservadora (infla N, deprime el DSR), pero ensancha una definición que gobierna una cifra publicada y la decidió un agente: queda declarada como norma de paso, con la asimetría que el `director-programa` señaló: la corrida de `bifurcaciones` de las 17:28, que sí produjo una re-medición de la ventaja, NO se contó como intento (se revirtió y quedó fuera del repo). Una de las dos está mal rotulada; cuál, lo decide Nicolás (`cola_decisiones.md`). Si rechaza la norma, `dinero/registro_intentos.py` vuelve a 3 con una línea.

**87.10 Desvío del pre-mortem, declarado.** El ítem 12 decía «4.1 y 4.2 apenas cierre el bloque 1, antes del 5»; el bloque 4 corrió en paralelo con el 5 y `ESTADO.md` se regeneró al cierre. Sin daño (el contador quedó leído de la base), pero no se cumplió como estaba escrito. Y una deuda que el director vio: el test de paridad numérica entre los dos README excluye la sección «Execution rail», la única sección sobre dinero de la portada, que sólo existe en inglés; su guardia es el contador desde el CSV (`test_el_contador_de_e0_del_readme_es_el_de_la_copia_versionada`), no la paridad. **`README.md` no debe publicarse (push) sin commitear `data/backups/sello_dinero.csv` en el mismo acto**: desde HEAD el generador da 8 / 7 / 2026-09-17.

**Deuda declarada.**
### Deuda: los literales del README fuera del árbitro
Hoy es inofensivo porque: el test de paridad numérica exige que los dos idiomas lleven exactamente los mismos tokens, y la procedencia de cada literal está censada en `dictamen_13/adversario_readme.md` §7.
Se vuelve peligroso cuando: un artefacto fuente (WS3, WS4, WS5, calibración) se recompute y el README no se mueva con él.
Bloquea: llamar «generado desde el árbitro» a la página entera; hoy lo es para la ventana sellada, la larga y E0.
Qué exigiría resolverla: extender `cifras.py` con esos estimadores y sus intervalos, con procedencia, o retirar del README lo que no entre.

## Acta §88, 19-sep-2026 (tarde). Firmas de Nicolás sobre lo que dejó abierto la corrida 13, el umbral de M2 (§43) y la ruta de IBKR (§54)

Redactada por el asistente del chat a partir de las respuestas de Nicolás del 19-sep-2026. Insumos a la vista al firmar: `bitacora_13.md`, `espera_firma.md` (tarjetas §43, §54, §58, §59 y §60) y, para 88.11 a 88.13, `GEMELO/resultados/m2_periodo.md`. `dictamen_13/` NO estuvo a la vista. Estado del repo verificado por Nicolás antes de firmar: `818ab99` (corrida 13) en `main` y en `origin/main`; único contenido sin versionar, las extensiones del 10 al 18 de septiembre en `dinero/datos/sello/`.

**Razón de Nicolás, con sus palabras, para 88.1 a 88.7 (de 88.8 en adelante cada punto trae la suya):** «concuerdo con todas las que recomendaste (esa es mi decision)». Las opciones y las recomendaciones etiquetadas que tuvo delante son las de las tarjetas citadas y las del mensaje del asistente de esa tarde. Donde existe una razón anterior de Nicolás, se cita en el punto.

**88.1. §58, hora del sello y definición de «completo»: opción (a) por ahora.** El timer del sellador sigue en `Mon..Fri 23:30 America/New_York` y la definición de §57 no cambia. Nicolás instala la sonda `mki-sonda-cierre` con la ventana alternativa `Mon..Fri 20..23:05,35 America/New_York` (no la `17..23`), para no descargar dentro de la ventana del riel de medición (17:50 a 20:30 Chile). La elección entre (b) y (c) se hace con 5 a 10 noches de dato de la sonda y acta nueva. Limitación declarada al firmar: la sonda termina a las 23:35 de Nueva York, así que tal como está no puede justificar una hora posterior a la actual; extenderla pasada la medianoche va al encargo 14 como propuesta. La unidad instalada difiere de la plantilla del repo en la hora de inicio; el encargo 14 alinea la plantilla.

**88.2. §59, `visible_en` y la zona ciega Z1: candidato (3).** Z1 no es un campo del sellador: es la pregunta del §23 / H1 (un tercero que recibe el sha antes de la apertura). No se agrega ningún campo al sellador. La zona ciega queda abierta y declarada hasta que se decida H1. §59 se cierra por remisión al §23.

**88.3. §60, hallazgo 2 de la revisión del 9-sep: opción (a), confirmada por escrito.** Una línea en `.gitignore` para `dinero/datos/sello/` y `git rm --cached` de `ext_2026-09-08.{csv,meta.json}` y `ext_2026-09-09.{csv,meta.json}`. `data/backups/sello_dinero_ext/` queda como única copia versionada. Razón de Nicolás del 19-sep: «para efectos de legibilidad». Condición: antes del `git rm --cached`, cada `fecha_insumo` sellada tiene su par commiteado en `data/backups/sello_dinero_ext/`; Nicolás lo verifica con `git ls-files` en el mismo acto. Costo aceptado: sale de git la ruta desde la que se restauró el 09-sep; lo compensa en parte el test permanente de integridad de la corrida 13.

**88.4. §87.9, registro de intentos del riel largo: se acepta el 4.** Queda como norma: un barrido descriptivo sobre el riel largo cuenta como intento aunque no pruebe una hipótesis sobre ventaja.

**88.5. Pregunta del guardián sobre la regla 10: sí alcanza.** Un archivo publicado nuevo que hereda cifras vencidas no las mueve solo; se corrigen con firma. Correcciones firmadas en este acto, para los dos README y sus plantillas: (i) los N de intentos 352/358 se reemplazan por los valores de los registros (354 y 360 según la bitácora 13; manda la máquina), de preferencia como marcadores que salen del registro; (ii) los badges `tests-650` y `plataforma-5.0.3` se reemplazan por los valores leídos de la máquina en la corrida que los aplique, de preferencia generados; (iii) el «59» de la rama derogada se corrige tal como lo describa `dictamen_13/adversario_readme.md`. Esta última firma es condicional: el asistente no vio ese dictamen; si no define un único valor correcto, vuelve como tarjeta y no se toca. El adversario verifica las tres antes de publicar.

**88.6. §87.4, `bifurcaciones`: las dos cosas, por separado.** (i) El artefacto publicado fija el nivel «vivo» al corte con el que se publicó (251 filas, 27-ago, según la bitácora 13), para que regenerarlo no mida otra cosa. (ii) La lectura sobre la cadena más larga es una medición nueva: diseño declarado antes de correr, dictamen del adversario, cuenta como intento, y no se publica sin ese dictamen. Las cifras que la corrida 13 vio y revirtió siguen NO DICTAMINADAS y no se citan.

**88.7. §43, M2: unidad.** M2 se lee como tasa anualizada (%/año), opción (c). El umbral, el deslizamiento y la ventana se firman en 88.11 a 88.13.

**88.8. §54, cliente de la API de IBKR: opción (a).** Nicolás instala el `ibapi` oficial desde el zip de IBKR aceptando la licencia no comercial; la instalación ocurre después de que él lea la licencia y concluya que un proyecto personal con capital propio cabe en ella. Entonces la línea comentada de `requirements.txt` se descomenta con la versión real. Razón de Nicolás: «por lo que se pierde al elegir la otra opcion» (con la Client Portal Web API se perdía el adaptador `corredor/ibkr.py` ya probado, y esa ruta estaba descartada en §84.4.2 por la sesión que expira).

**88.9. Dónde corre IB Gateway: dentro de WSL, con la versión Linux.** El adaptador se conecta a `127.0.0.1` y el guardia de puerto y de prefijo «DU» queda como está. Razón de Nicolás: «por comodidad, prefiero todo contenido dentro de linux». Verificado por Nicolás el 19-sep: WSLg presente (`/mnt/wslg` existe, `DISPLAY=:0`).

**88.10. Estado de la cuenta de IBKR al 19-sep-2026 (captura de Nicolás).** Solicitud completa y fondeada con 5,00 USD, **todavía en revisión**: IBKR avisa por correo al aprobar. E1 se hace en la cuenta de práctica y no exige más depósito; el monto de E2 sigue siendo el de §86.3 y no se deposita antes de E1 completo y la vara cumplida. E1 no empieza hasta el correo de aprobación.

**88.11. §43, umbral de M2: 8,3 %/año** (25 % del capital aportado repartido en 156 semanas, es decir 25/3). Razón de Nicolás: «pq se deriva de mi numero original». Lo que tuvo delante (`m2_periodo.md`, K = 20 semillas, PROPUESTA, una sola trayectoria de mercado, señal sin información, arancel del §40 sin firma): tasa anualizada mediana del conservador entre 3,9 % y 4,3 % según horizonte (mín a máx entre semillas 2,09 a 4,75), agresivo 6,2 % a 6,4 % (5,66 a 7,83), medio 8,9 % a 9,4 % (4,69 a 11,18). Consecuencia aceptada: bajo este umbral el juego medio dispara en la mediana simulada. **Grado de libertad declarado:** el umbral se fijó con esas lecturas a la vista y con 12 lecturas ya computadas (E12 del dictamen 12), igual que se declaró para el monto en §86.3.

**88.12. M2 y el deslizamiento: no cuenta.** M2 se lee sólo de las comisiones que informa el corredor; el deslizamiento se registra aparte. Razón de Nicolás: «coincido con tu opinion» (la recomendación etiquetada decía que una regla de apagado debe leerse de un número que no admite discusión, y el deslizamiento exige estimar un precio de referencia).
