...
|---|---|---|---|---|---|---|
| **Tokio** (XTKS) | {{larga_tokio_n}} | 72.9% | 53.8% | **{{larga_tokio_ventaja_pp}} pp** | ≈0 | **{{larga_tokio_margen_h}} h** |
| **Taipéi** (XTAI) | {{larga_taipei_n}} | 72.0% | 55.2% | **{{larga_taipei_ventaja_pp}} pp** | ≈0 | **{{larga_taipei_margen_h}} h** |
| **Seúl** (XKRX) | {{larga_seul_n}} | 71.2% | 55.8% | **{{larga_seul_ventaja_pp}} pp** | ≈0 | **{{larga_seul_margen_h}} h** |
| **Fráncfort** (XETR) | {{larga_francfort_n}} | 57.2% | 54.7% | **{{larga_francfort_ventaja_pp}} pp** | **{{larga_p_francfort}}** | **{{larga_francfort_margen_h}} h** |
...
|---|---|---|
| **Modelo 4.6.0** | **{{modelo_pct}}%** ({{modelo_aciertos}}/{{n}}) | {{modelo_wilson}} |
| **"Siempre al alza", mismas filas** | **{{base_pct}}%** ({{base_aciertos}}/{{n}}) | {{base_wilson}} |
| **Ventaja** | **{{ventaja_pp}} pp** | IC95 de día **{{ventaja_ic_dia}}** · McNemar p = {{mcnemar_p}} (χ² con corrección de continuidad; binomial exacta {{mcnemar_p_exacta}}); percentil de día con cobertura medida ~0.93 (`calibracion_instrumento.md` A1), t de clúster {{ventaja_ic_t_cluster}} |
...
es el día ({{dias}} días, ICC {{icc}}, DEFF {{deff}}, ~{{n_efectivo}} observaciones efectivas), y
con esa unidad el intervalo contiene el cero y la permutación de signo por
día da p = {{p_permutacion_dia}}. El McNemar de filas se publica al lado porque es el test
del diseño original, no porque decida (acta §61).

**CONTESTADA: la ventaja no es capturable.** Entrar en la apertura y salir al
cierre pierde 40,7 % sin un solo punto básico de costo y 95,6 % con 25 pb por
lado, contra +137,1 % de comprar el ETF del sector (acta §59;
`estado_epistemico.md` §10). El gap existe; el retorno de sesión no lo sigue.
...
| Otras métricas (n={{n}}) | Valor | Caveat honesto |
|---|---|---|
| Acierto del retorno de sesión | {{retorno_pct}}% · IC95 {{retorno_wilson}} (n={{retorno_n}}) | un solo régimen observado |
| **MAE del gap** | **{{mae_modelo_pp}} pp** vs **{{mae_cero_pp}}** de predecir cero | ganancia {{mae_ganancia_pp}} pp por fila, IC95 t de clúster de día {{mae_ganancia_ic_t_dia}}, p de día {{mae_ganancia_p_dia}}: **contiene el cero, no distinguible al nivel de día**; parte de la mejora respecto de la rama retirada es que la regla saca filas con gaps enormes del 29-jul |
| Cobertura del intervalo 80% | {{cobertura_80_pct}}% (nominal 80%) | intervalos **{{ratio_ancho}}× más anchos** de lo necesario (IC95 de día {{ratio_ancho_ic_dia}}) |
...
ventaja es {{larga_ventaja_pp}} pp.**
