      ...
      <table className="w-full border-collapse text-xs">
        <thead>
          <tr className="border-b border-border-strong">
            <th className="px-2 py-2 text-left text-[11px] font-medium uppercase tracking-wider text-text-3" />
            {columnas.map((c) => (
              <th
                key={c}
                className="num px-2 py-2 text-right text-[11px] font-medium text-text-3"
              >
                {c}
              </th>
            ))}
        ...
        <tbody>
          {filas.map((f) => (
            <tr key={f.nombre} className="border-b border-border last:border-0">
              <td className="px-2 py-2 text-text-2">{f.nombre}</td>
              {f.valores.map((v, i) => (
                <td
                  key={i}
                  className="num px-2 py-2 text-right text-text-1"
